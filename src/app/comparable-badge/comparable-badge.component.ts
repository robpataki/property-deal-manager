...
@Component({
  selector: 'app-comparable-badge',
  template: `
  <span class="comparable-badge" #comparableBadge>
    <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid" viewBox="0 0 24 24" width="24" height="24">
  ...
  styleUrls: ['./comparable-badge.component.scss']
})
export class ComparableBadgeComponent implements AfterViewInit {
  @Input() size: string = '';
  @ViewChild('comparableBadge') el: ElementRef<HTMLElement>;

  ngAfterViewInit() {
    if (!!this.size && this.size.length) {
      this.el.nativeElement.classList.add(`comparable-badge--${this.size}`);
    }
    console.log();
  }
}